...
struct LaunchpadView: View {
  ...
  @State private var folderHoverCandidateIndex: Int?
  @State private var folderHoverBeganAt: Date?
  @State private var selectedIndex: Int?
  @State private var isKeyboardNavigationActive: Bool = false
  @FocusState private var isSearchFieldFocused: Bool
  @Namespace private var reorderNamespace
  @State private var handoffEventMonitor: Any?
  ...
  private static var lastGeometryUpdate: Date = .distantPast
  ...
  private var visualItems: [LaunchpadItem] {
    ...
    while pageSlices.count <= targetPage {
      pageSlices.append([])
    }
  ...
  var body: some View {
    ...
    .onDisappear {
      for monitor in [keyMonitor, handoffEventMonitor] {
        if let monitor { NSEvent.removeMonitor(monitor) }
      }
      if let monitor = globalMouseUpMonitor { NSEvent.removeMonitor(monitor) }
      for observer in [windowObserver, windowHiddenObserver] {
  ...
  private func finalizeHandoffDrag() {
    guard draggingItem != nil else { return }
    defer {
      if let monitor = handoffEventMonitor { NSEvent.removeMonitor(monitor)
        handoffEventMonitor = nil
      }
  ...
  private func startFPSMonitoring() {
    ...
    if let monitor = FPSMonitor { fps, frameDelta in
      ...
      DispatchQueue.main.async {
        ...
        fpsValue = smoothed
        frameTimeMilliseconds = frameDelta * 1_000
      }
    } {
      fpsMonitor = monitor
    }
...
private extension LaunchpadView {

  func closeIfTappedOnEmptyOrGap(at point: CGPoint,
                                 geoSize: CGSize,
                                 columnWidth: CGFloat,
                                 appHeight: CGFloat,
                                 iconSize: CGFloat) {
...
extension LaunchpadView {
  ...
  private func handleControllerCommand(_ command: ControllerCommand) {
    ...
    switch command {
    case let .move(direction), let .moveRepeat(direction):
...
struct ScrollEventCatcher: NSViewRepresentable {
  ...
  func makeNSView(context _: Context) -> ScrollEventCatcherView {
    let view = ScrollEventCatcherView()
    view.onScroll = onScroll
    return view
  }

  func updateNSView(_ nsView: ScrollEventCatcherView, context _: Context) {
...
private extension LaunchpadView {
  ...
  func captureGridGeometry(_ geo: GeometryProxy, columnWidth: CGFloat, appHeight: CGFloat, iconSize: CGFloat) {
  ...
  func flipPageIfNeeded(iconCenter: CGPoint,
                        pointer: CGPoint,
                        iconSize: CGFloat,
  ...
  func predictedDropIndex(for pointer: CGPoint, in containerSize: CGSize, columnWidth: CGFloat, appHeight: CGFloat) -> Int? {
...
struct GridConfig {
  ...
  init(isFullscreen: Bool = false,
    ...
    rowCount = max(1, rows)
    columnSpacingValue = max(0, columnSpacing)
    rowSpacingValue = max(0, rowSpacing)
  }
...
extension LaunchpadView {
  ...
  private func finalizeDragOperation(containerSize: CGSize, columnWidth: CGFloat, appHeight: CGFloat, iconSize _: CGFloat) {
    ...
    // 处理文件夹创建逻辑
    if appStore.isDragCreatingFolder, case let .app(app) = dragging {
      if let targetApp = appStore.folderCreationTarget {
        ...
        if let hoveringIndex = indexAt(point: pointerPoint,
          ...
          case let .folder(folder) = filteredItems[hoveringIndex] {
  ...
  private func handleMissingHover(dragging: LaunchpadItem,
                                  ...
                                  hoveringIndex: Int,
                                  isInCenterArea _: Bool) {
    appStore.isDragCreatingFolder = false
    appStore.folderCreationTarget = nil
    if case let .missingApp(draggingPlaceholder) = dragging,
  ...
  private func measurePerformance<T>(_ operation: String, _ block: () -> T) -> T {
    ...
    if timeElapsed > 0.016 { // 超过16ms（60fps阈值）
      print("⚠️ 性能警告: \(operation) 耗时 \(String(format: "%.3f", timeElapsed * 1_000))ms")
    }
