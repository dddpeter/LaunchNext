...
enum AppearancePreference: String, CaseIterable, Identifiable {
  ...
  var localizationKey: LocalizationKey {
    switch self {
    case .system: .appearanceModeFollowSystem
...
final class AppStore: ObservableObject {
  ...
  enum BackgroundStyle: String, CaseIterable, Identifiable {
    ...
    var localizationKey: LocalizationKey {
      switch self {
      case .blur: .backgroundStyleOptionBlur
      case .glass: .backgroundStyleOptionGlass
      }
    }
  ...
  struct HotKeyConfiguration: Equatable {
    ...
    init(keyCode: UInt16, modifierFlags: NSEvent.ModifierFlags) {
      ...
      modifiersRawValue = modifierFlags.normalizedShortcutFlags.rawValue
    }
    ...
    init?(dictionary: [String: Any]) {
      ...
      modifiersRawValue = NSEvent.ModifierFlags.RawValue(rawModifiers)
    }
    ...
    private static func keyDisplayName(for keyCode: UInt16) -> String {
      ...
      guard let layout = TISCopyCurrentKeyboardLayoutInputSource()?.takeRetainedValue(),
            let rawPtr = TISGetInputSourceProperty(layout, kTISPropertyUnicodeKeyLayoutData)
      else {
    ...
    private static func fallbackName(for keyCode: UInt16) -> String {
      specialKeyNames[keyCode] ?? String(format: "Key %d", keyCode)
    }
  ...
  @Published var folderCreationTarget: AppInfo?
  @Published var openFolderActivatedByKeyboard: Bool = false
  @Published var isFolderNameEditing: Bool = false
  @Published var handoffDraggingApp: AppInfo?
  @Published var handoffDragScreenLocation: CGPoint?

  // 触发器
  @Published var folderUpdateTrigger: UUID = .init()
  @Published var gridRefreshTrigger: UUID = .init()
  ...
  private func shouldTrackMissingPlaceholder(at _: String,
    ...
    if !removalSet.isEmpty {
      ...
      if !hiddenAppPaths.isEmpty {
        updateHiddenAppPaths { hidden in
          for path in removalSet {
            hidden.remove(path)
          }
          for raw in removalRawPaths {
            hidden.remove(raw)
          }
        }
      }
  ...
  init() {
    if UserDefaults.standard.object(forKey: "isFullscreenMode") == nil {
      isFullscreenMode = true // 新用户默认 Classic (Fullscreen)
    ...
    iconColumnSpacing = clampedColumnSpacing
    ...
    iconRowSpacing = clampedRowSpacing
    ...
    folderDropZoneScale = clampedDropZoneScale
    ...
    pageIndicatorTopPadding = clampedTopPadding
    ...
    rememberLastPage = shouldRememberPage
    if shouldRememberPage, let savedPageIndex {
      currentPage = max(0, savedPageIndex)
    }
  ...
  private func processScannedApplications(_ newApps: [AppInfo]) {
    ...
    let existingPaths = Set(updatedApps.map(\.url.path))
    ...
    // 第四步：智能重建项目列表，保持用户排序
    smartRebuildItemsWithOrderPreservation(currentItems: currentItems, newApps: newAppsToAdd)
  ...
  private func rebuildItemsWithStrictOrderPreservation(currentItems: [LaunchpadItem]) {
    ...
    let appsInFolders = Set(folders.flatMap(\.apps))
    ...
    for (_, item) in currentItems.enumerated() {
      ...
      case let .app(app):
        ...
        // 检查应用是否仍然存在
        if apps.contains(where: { standardizedFilePath($0.url.path) == standardizedPath }) {
  ...
  private func smartRebuildItemsWithOrderPreservation(currentItems: [LaunchpadItem], newApps: [AppInfo]) {
    ...
    let hasPersistedData = hasPersistedOrderData()

    if hasPersistedData {
      // 智能合并现有顺序和持久化数据
      mergeCurrentOrderWithPersistedData(currentItems: currentItems, newApps: newApps, loadPersistedFolders: true)
    } else {
      // 没有持久化数据时，直接基于当前顺序合并
      mergeCurrentOrderWithPersistedData(currentItems: currentItems, newApps: newApps, loadPersistedFolders: false)
    }
  }
  ...
  /// 智能合并现有顺序和持久化数据
  private func mergeCurrentOrderWithPersistedData(currentItems: [LaunchpadItem], newApps _: [AppInfo], loadPersistedFolders: Bool = true) {
    // 保存当前的项目顺序
  ...
  private func loadFoldersFromPersistedData() {
    ...
    do {
      ...
      if !saved.isEmpty {
        ...
        folders = sanitizedFolders(foldersInOrder)
      }
    } catch {}
  }
  ...
  private func setupVolumeObservers() {
    ...
    let mountObserver = center.addObserver(forName: NSWorkspace.didMountNotification, object: nil, queue: .main) { [weak self] notification in
      ...
      handleVolumeEvent(at: url, isMount: true)
    }
    ...
    let unmountObserver = center.addObserver(forName: NSWorkspace.didUnmountNotification, object: nil, queue: .main) { [weak self] notification in
      guard let self, let url = notification.userInfo?[NSWorkspace.volumeURLUserInfoKey] as? URL else { return }
      handleVolumeEvent(at: url, isMount: false)
    }
  ...
  private func applyIncrementalChanges(for changedPaths: Set<String>) {
    ...
    refreshQueue.async { [weak self] in
      ...
      for path in changedPaths {
        ...
        let valid = exists && isValidApp(at: url) && !isInsideAnotherApp(url)
        if valid {
      ...
      DispatchQueue.main.async { [weak self] in
        ...
        // 应用更新
        let updates: [AppInfo] = changes.compactMap { if case let .update(info) = $0 { info } else { nil } }
        if !updates.isEmpty {
          var map: [String: Int] = [:]
          ...
          for info in updates {
            let standardizedInfoPath = standardizedFilePath(info.url.path)
            if let idx = map[info.url.path], apps.indices.contains(idx) { apps[idx] = info }
            for fIdx in folders.indices {
              for aIdx in folders[fIdx].apps.indices where folders[fIdx].apps[aIdx].url.path == info.url.path {
                self.folders[fIdx].apps[aIdx] = info
              }
            }
            for iIdx in items.indices {
              switch items[iIdx] {
              case let .app(a):
                if standardizedFilePath(a.url.path) == standardizedInfoPath {
                  items[iIdx] = .app(info)
                  clearMissingPlaceholder(for: standardizedInfoPath)
                }
              case let .missingApp(placeholder):
                if standardizedFilePath(placeholder.bundlePath) == standardizedInfoPath {
                  items[iIdx] = .app(info)
                  clearMissingPlaceholder(for: standardizedInfoPath)
                }
              default:
        ...
        // 新增应用
        let inserts: [AppInfo] = changes.compactMap { if case let .insert(info) = $0 { info } else { nil } }
        if !inserts.isEmpty {
          apps.append(contentsOf: inserts)
          apps.sort { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
          rebuildItems()
        }
  ...
  private func isInsideAnotherApp(_ url: URL) -> Bool {
    let appCount = url.pathComponents.count(where: { $0.hasSuffix(".app") })
    return appCount > 1
  }
  ...
  func createFolder(with apps: [AppInfo], name: String = "Untitled") -> FolderInfo {
    createFolder(with: apps, name: name, insertAt: nil)
  }
  ...
  func createFolder(with apps: [AppInfo], name: String = "Untitled", insertAt insertIndex: Int?) -> FolderInfo {
    ...
    let baseIndex = placeholders.map(\.0).min() ?? min(newItems.count - 1, max(0, insertIndex ?? (newItems.count - 1)))
  ...
  func addAppToFolder(_ app: AppInfo, folder: FolderInfo) {
    ...
    for idx in items.indices {
      if case let .folder(f) = items[idx], f.id == updatedFolder.id {
  ...
  func removeAppFromFolder(_ app: AppInfo, folder: FolderInfo) {
    ...
    for idx in items.indices {
      if case let .folder(f) = items[idx], f.id == folder.id {
        if updatedFolder.apps.isEmpty {
    ...
    var targetSlot: Int? = if let firstEmptied = emptiedSlots.first, firstEmptied < items.count {
      firstEmptied
    } else {
      items.firstIndex {
  ...
  func compactItemsWithinPages() {
    ...
    let itemsPerPage = itemsPerPage // 使用计算属性
  ...
  func rebuildItems() {
    ...
    let appsInFolders: Set<AppInfo> = Set(folders.flatMap(\.apps))
    ...
    for item in items {
      ...
      case let .missingApp(placeholder):
        if let item = currentMissingAppItem(for: placeholder) {
          newItems.append(item)
          if case let .missingApp(current) = item {
  ...
  private func loadOrderFromPageEntries(using modelContext: ModelContext) -> Bool {
    do {
      ...
      let folderAppPathSet: Set<String> = Set(foldersInOrder.flatMap { $0.apps.map(\.url.path) })
      ...
      DispatchQueue.main.async {
        ...
        if !combined.isEmpty {
          ...
          if self.apps.isEmpty {
            let freeApps: [AppInfo] = combined.compactMap { if case let .app(a) = $0 { a } else { nil } }
            self.apps = freeApps
            self.pruneHiddenAppsFromAppList()
          }
        }
  ...
  private func loadOrderFromLegacyTopItems(using modelContext: ModelContext) {
    do {
      ...
      var folderMap: [String: FolderInfo] = [:]
      var foldersInOrder: [FolderInfo] = []
      let folderAppPathSet: Set<String> = Set(saved.filter { $0.kind == "folder" }.flatMap(\.appPaths))
      for row in saved where row.kind == "folder" {
        let folderApps: [AppInfo] = row.appPaths.compactMap { path in
      ...
      let appsInFolders = Set(foldersInOrder.flatMap(\.apps))
      ...
      DispatchQueue.main.async {
        ...
        if !combined.isEmpty {
          ...
          // 如果应用列表为空，从持久化数据中恢复应用列表
          if self.apps.isEmpty {
            let freeAppsAfterLoad: [AppInfo] = combined.compactMap { if case let .app(a) = $0 { a } else { nil } }
      ...
      for (idx, item) in items.enumerated() {
        ...
        case let .folder(folder):
          ...
          let row = PageEntryData(
            ...
            appPaths: authoritativeFolder.apps.map(\.url.path),
          )
  ...
  func cleanupUnusedNewPage() {
    ...
    if pageStart < items.count {
      ...
      let hasNonEmptyItems = pageSlice.contains { item in
        if case .empty = item { false } else { true }
      }
  ...
  func removeEmptyPages() {
    ...
    while index < items.count {
      ...
      let isEmptyPage = pageSlice.allSatisfy { item in
        if case .empty = item { true } else { false }
      }
  ...
  private func buildExportData() -> [String: Any] {
    ...
    for (index, item) in items.enumerated() {
      ...
      if case let .folder(folder) = item {
        itemData["folderApps"] = folder.apps.map(\.name)
        itemData["folderAppPaths"] = folder.apps.map(\.url.path)
      }
  ...
  private func itemPath(for item: LaunchpadItem) -> String {
    ...
    case .empty:
      "空槽位"
    case let .missingApp(placeholder):
      "缺失应用: \(placeholder.bundlePath)"
    }
  }

  /// 使用系统文件保存对话框保存导出文件
  func saveExportFileWithDialog(content: String, filename: String, fileExtension: String, fileType _: String) -> Bool {
  ...
  private func generateCacheAfterScan() {
    ...
    if !cacheManager.isCacheValid {
      ...
      // 缓存有效，但可以预加载图标
      let appPaths = apps.map(\.url.path)
  ...
  private func updateCacheAfterChanges() {
    ...
    if !cacheManager.isCacheValid {
      ...
      // 缓存有效，只更新变化的部分
      let changedAppPaths = apps.map(\.url.path)
  ...
  private func applyCustomTitleOverride(for url: URL, title: String?) {
    ...
    for itemIndex in items.indices {
      ...
      case let .app(app) where app.url == url:
  ...
  private func applyCurrentAppIcon() {
    ...
    DispatchQueue.main.async {
      ...
      let success: Bool = if hasCustomIconFile {
        workspace.setIcon(icon, forFile: bundlePath, options: [])
      } else {
        workspace.setIcon(nil, forFile: bundlePath, options: [])
      }
  ...
  private func processImportedData(_ importData: Any) -> Bool {
      ...
      case "文件夹":
        if let folderApps = pageData["folderApps"] as? [String],
          ...
          if !folderAppsList.isEmpty {
            ...
            let existingFolder = folders.first { existingFolder in
    ...
    let usedAppsInFolders = Set(importedFolders.flatMap(\.apps))
  ...
  private func presentUpdaterLaunchFailure(_ error: Error) {
    ...
    let detail: String = if let launchError = error as? UpdaterLaunchError {
      switch launchError {
      case .missingBinary:
        localized(.updaterMissingBinary)
      case .notExecutable:
        localized(.updaterNotExecutable)
      case let .spawnFailed(underlying):
