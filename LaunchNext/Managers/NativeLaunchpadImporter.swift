...
class NativeLaunchpadImporter {
  ...
  private func convertAndSave(launchpadData: LaunchpadData) throws -> ImportResult {
    ...
    #if DEBUG
      print("🧭 顶层容器顺序: \(topContainers.map(\.rowId).joined(separator: ", "))")
    #endif
    ...
    for (pageIndex, container) in topContainers.enumerated() {
      ...
      let maxPos = max(directApps.map(\.ordering).max() ?? -1,
                       folderPages.map(\.ordering).max() ?? -1)
      ...
      for page in folderPages {
        ...
        if isPlaceholderFolderTitle(folderNameRaw) {
          ...
          if top.isEmpty { finalName = "Untitled" } else if top.count == 1 { finalName = top[0] } else if top.count == 2 { finalName = top[0] + " + " + top[1] } else { finalName = top[0] + " + " + top[1] + " + …" }
        } else {
  ...
  private func buildHierarchy(from data: LaunchpadData) -> LaunchpadHierarchy {
    ...
    let rootIds: [Int] = if roots.isEmpty {
      [1] // 兜底：典型旧库中 root 为 1
    } else {
      // 按 ordering 排序（若无意义，则自然顺序）
      roots.sorted { $0.ordering < $1.ordering }.map { intValue($0.rowId) }
    }
    ...
    #if DEBUG
      let tcIds = topContainers.map(\.container.rowId)
    ...
    #if DEBUG
      let pageIds = orderedPages.map(\.rowId)
  ...
  private func computeFolderName(from apps: [AppInfo]) -> String {
    let names = apps.prefix(3).map(\.name)
    switch names.count {
...
enum ImportError: LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case let .databaseNotFound(msg):
      "Database not found: \(msg)"
    case let .databaseError(msg):
      "Database error: \(msg)"
    case let .systemError(msg):
      "System error: \(msg)"
    case let .conversionError(msg):
