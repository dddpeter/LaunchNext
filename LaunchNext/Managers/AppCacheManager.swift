...
final class AppCacheManager: ObservableObject {
  ...
  func generateCache(from apps: [AppInfo],
    ...
    DispatchQueue.global(qos: .userInitiated).async { [weak self] in
      ...
      // 缓存应用信息
      cacheAppInfos(uniqueApps)

      // 缓存应用图标
      cacheAppIcons(uniqueApps)

      // 缓存网格布局数据
      cacheGridLayout(items,
  ...
  func preloadIcons(for appPaths: [String]) {
    DispatchQueue.global(qos: .utility).async { [weak self] in
      ...
      for path in appPaths {
        if getCachedIcon(for: path) == nil {
          let icon = NSWorkspace.shared.icon(forFile: path)
          let key = cacheKeyGenerator.generateIconKey(for: path)
          cacheLock.lock()
          iconCache[key] = icon
          iconCacheOrder.append(key)
  ...
  private func calculatePageInfo(for items: [LaunchpadItem], itemsPerPage: Int) -> [PageInfo] {
    ...
    for pageIndex in 0 ..< pageCount {
      ...
      let appCount = pageItems.count(where: { if case .app = $0 { true } else { false } })
      let folderCount = pageItems.count(where: { if case .folder = $0 { true } else { false } })
      let emptyCount = pageItems.count(where: { if case .empty = $0 { true } else { false } })
  ...
  var performanceStats: PerformanceStats {
    PerformanceStats(cacheSize: cacheSize)
  }
