...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSGestureRecognizerDelegate {
  ...
  func applicationDidFinishLaunching(_: Notification) {
    ...
    DispatchQueue.main.async { [weak self] in
      ...
      applyAppearancePreference(appStore.appearancePreference)
  ...
  func presentLaunchError(_: Error, for _: URL) {}
  ...
  func quitWithFade() {
    guard !isTerminating else { NSApp.terminate(nil)
      return
    }
    isTerminating = true
    if let window {
  ...
  func windowDidResignKey(_: Notification) { autoHideIfNeeded() }
  func windowDidResignMain(_: Notification) { autoHideIfNeeded() }
  ...
  func gestureRecognizer(_: NSGestureRecognizer, shouldAttemptToRecognizeWith event: NSEvent) -> Bool {
...
private func hotKeyEventCallback(eventHandlerCallRef _: EventHandlerCallRef?, event _: EventRef?, userData: UnsafeMutableRawPointer?) -> OSStatus {
